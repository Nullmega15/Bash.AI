...
# Max chat history length to send to AI for context
MAX_CHAT_HISTORY_LENGTH = 10 # Keep last 10 messages (5 user, 5 assistant)
...
class Spinner:
    ...
    def stop(self):
        ...
        # Ensure the line is cleared even if thread wasn't running or join timed out
        sys.stdout.write("\r" + " " * (os.get_terminal_size().columns) + "\r")
        sys.stdout.flush()

    # Context manager methods for convenience, still use explicit start/stop for input prompts
    def __enter__(self):
...
class BashAI:
    ...
    def __init__(self, server_url: str = None):
        ...
        self.history = [] # Command history for interactive mode (for readline)
        self.chat_history: List[Dict[str, str]] = [] # Chat history for AI context
        # self.last_ai_suggestion: Optional[Dict[str, Any]] = None # REMOVED: Stores the last actionable AI suggestion
        self.supabase_client: Optional[Client] = None # Supabase client instance
        self.jwt_token: Optional[str] = None # User's JWT token
        self.spinner = Spinner() # Initialize the global spinner instance
    ...
    def _authenticate_user(self):
        """
        Prompts the user to log in with Supabase.
        ...
        print(f"To unlock full features, please log in.")

        # Only prompt for login
        self.spinner.stop() # Stop spinner before input
    ...
    def _save_config(self, config_data: Dict):
        ...
        try:
            ...
            with open(CONFIG_PATH, 'w', encoding='utf-8') as f: # Added encoding='utf-8'
    ...
    def _query_ai(self, message: str, system_prompt: str = None) -> Tuple[str, bool]:
        """
        Sends a query to the AI server and retrieves the response, including JWT and chat history.
        """
        headers = {
        ...
        try:
            self.spinner.start("Thinking") # Start spinner before AI query
            payload = {
                "message": message,
                "system_prompt": system_prompt,
                "max_tokens": 30000, # Max tokens for AI response
                "temperature": 0.7, # Creativity of AI response
                "chat_history": self.chat_history # Include chat history for context
            }
            ...
            self.spinner.stop() # Ensure spinner is stopped after AI query
    ...
    def _execute_command(self, cmd: str, show_command: bool = True) -> Tuple[str, bool]:
        ...
        process = None
        try:
            self.spinner.start("Running") # Start spinner before command execution
            process = subprocess.run(
                full_cmd, # Use the potentially modified full_cmd
                shell=True, # Still use shell=True for general parsing by the system shell
                capture_output=True,
                text=True,
                creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, 'CREATE_NO_WINDOW') else 0
            )
            self.spinner.stop() # Stop spinner after command completes

            if process.returncode == 0:
                output = process.stdout.strip() if process.stdout else "✓ Command completed successfully"
                return output, True
            else:
                error = process.stderr.strip() if process.stderr else f"Command failed with exit code {process.returncode}"
                print(f"{Colors.RED}Command failed (Exit Code: {process.returncode}).{Colors.END}")
                if error:
                    print(f"{Colors.RED}Stderr:\n{error}{Colors.END}")
                
                self.spinner.stop() # Ensure spinner is stopped before input
                debug_choice = input(f"{Colors.YELLOW}Attempt to debug this command with AI? [y/N]: ").strip().lower()
                if debug_choice == 'y':
                    return self._debug_command_error(cmd, error) # Pass original cmd to debugger
                else:
            ...
            self.spinner.stop() # Ensure spinner is stopped on error
...
"""
    ...
    def _read_output_stream(self, stream, output_list):
        ...
        and print them in real-time, also collecting them into a list of decoded strings.
        """
        for line_bytes in iter(stream.readline, b''):
            try:
                # Decode the bytes to string using the system's preferred encoding
                line_str = line_bytes.decode(sys.stdout.encoding, errors='replace')
                # Print the line directly to stdout
                sys.stdout.write(line_str)
                sys.stdout.flush() # Ensure immediate display
                output_list.append(line_str) # Append decoded string to list
            except Exception as e:
                sys.stderr.write(f"Error decoding/writing stream: {e}\n")
                sys.stderr.flush()
                break # Exit on error
    ...
    def _run_code_file(self, filename: str, code_content: str, dependencies_cmd: Optional[str] = None) -> bool:
        ...
        # We will collect full output after the process finishes for debugging,
        # but the _read_output_stream will handle real-time printing.
        stdout_lines = [] # Now stores decoded strings
        stderr_lines = [] # Now stores decoded strings

        try:
            self.spinner.start("Running") # Start spinner before execution
            process = subprocess.Popen(
                command_parts,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=False, # Read as bytes, decode in thread
                creationflags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess, 'CREATE_NO_WINDOW') else 0
            )
            ...
            while process.poll() is None: # While the child process is still running
                try:
                    ...
                    if user_input == 'stop':
                        ...
                        break # Exit the loop
                    self.spinner.start("Running") # Restart spinner after input
            ...
            if process:
                try:
                    ...
                    process.wait(timeout=2) # Increased wait time
                except subprocess.TimeoutExpired:
                    print(f"{Colors.RED}Process did not terminate gracefully within timeout. Killing...{Colors.END}")
                    if process.poll() is None: # Only if still running
                        process.kill() # Force kill if terminate fails
                    process.wait(timeout=2) # Wait again after kill

                # Explicitly close the pipes *before* joining threads
                # This should unblock the reading threads
                if process.stdout:
                ...
                if stdout_thread and stdout_thread.is_alive():
                    stdout_thread.join(timeout=2) # Increased join timeout
                if stderr_thread and stderr_thread.is_alive():
                    stderr_thread.join(timeout=2)
                
                # If threads are still alive after join, it means they are stuck.
                # In such a case, we can't do much more but acknowledge.
                if stdout_thread and stdout_thread.is_alive():
                    print(f"{Colors.YELLOW}Warning: stdout reading thread did not terminate.{Colors.END}")
                if stderr_thread and stderr_thread.is_alive():
                    print(f"{Colors.YELLOW}Warning: stderr reading thread did not terminate.{Colors.END}")

            self.spinner.stop() # Final ensure spinner is stopped


        # After execution (or termination), join the collected lines
        full_stdout = "".join(stdout_lines)
        full_stderr = "".join(stderr_lines)

        if process and process.returncode == 0:
            print(f"\n{Colors.GREEN}✓ Code execution completed successfully.{Colors.END}")
            # The output has already been printed in real-time by the thread.
            # No need to print full_stdout here again unless for debugging.
            return True
        elif process and process.returncode is not None:
            print(f"\n{Colors.RED}Code execution failed (Exit Code: {process.returncode}).{Colors.END}")
            # The error output has already been printed in real-time by the thread.
            # No need to print full_stderr here again unless for debugging.
        ...
        for attempt in range(1, DEBUG_ATTEMPTS + 1):
            ...
            if parsed_debug_response['command']:
                ...
                if attempt < DEBUG_ATTEMPTS:
                    ...
                    print(f"{Colors.RED}Manual intervention may be required.{Colors.END}")
                    return f"{Colors.RED}Command failed and AI could not debug.{Colors.END}", False # Fallback if all attempts fail
    ...
    def _interactive_mode(self):
        ...
        while True:
            try:
                ...
                user_input = input(prompt).strip() # No spinner here, just the main prompt
                ...
                # Add AI's raw response to chat history for context
                self.chat_history.append({"role": "assistant", "content": ai_response_raw})
                # Trim chat history again after adding AI response
                if len(self.chat_history) > MAX_CHAT_HISTORY_LENGTH:
                    self.chat_history = self.chat_history[-MAX_CHAT_HISTORY_LENGTH:]

                # --- Process AI Response: Immediately prompt for action ---
                ...
                # 3. Handle Edited File
                elif parsed['edited_file']: # Use elif to prioritize 'new_files' if both present
                ...
                if parsed['command']: # Note: 'if' not 'elif' to allow execution after file creation/edit
    ...
    def _show_help(self):
        ...
        # print(f"{Colors.YELLOW}New: Type 'do it' to execute the last suggested action by the AI.{Colors.END}") # REMOVED THIS LINE
...
def main():
    ...
    if args.command:
        ...
        # In single command mode, we don't maintain chat history across calls
